...
$(function () {
	...
	$("#chageOne").on("click", function () { // 출장완료로
		$("#isChangeOne").show();
		$("#changeOneOk").on("click", function () {
			self.location = "/ash/updateMyReservationCondition?cancel=0&ashReservationNo="+${ash.ashReservationNo};
		})
		$("#changeOneNo").on("click", function () {
			$("#isChangeOne").hide();
		})
	})
	$("#chageTwo").on("click", function () { //출장 확정으로
		$("#isChangeTwo").show();
		$("#changeTwoOk").on("click", function () {
			self.location = "/ash/updateMyReservationCondition?cancel=0&ashReservationNo="+${ash.ashReservationNo}
		})
		$("#changeTwoNo").on("click", function () {
			$("#isChangeTwo").hide();
		})
	})
})
...
<!-- 						<img src="http://placehold.it/300x300" alt=""/> -->
				...
				<c:if test="${ash.ashReservationCondition==0}">
					...
					<a style="cursor: pointer;" id="chageOne" class="highlight-button-dark2 btn btn-medium no-margin-right no-margin-bottom margin-two btn-round">출장확정으로 변경</a>
				</c:if>
				<c:if test="${ash.ashReservationCondition==1}">
					<a style="cursor: pointer;" id="updateReservation" class="highlight-button-dark2 btn btn-medium no-margin-right no-margin-bottom margin-two btn-round">예약수정</a>
					<a style="cursor: pointer;" id="cancelReservation" class="highlight-button-dark2 btn btn-medium no-margin-right no-margin-bottom margin-two btn-round">예약취소</a>
					<a style="cursor: pointer;" id="chageTwo" class="highlight-button-dark2 btn btn-medium no-margin-right no-margin-bottom margin-two btn-round" >출장완료로 변경</a>
				</c:if>
				<c:if test="${ash.ashReservationCondition==3}">고객님이 예약을 취소하셨습니다.</c:if>
				<c:if test="${ash.ashReservationCondition==2}">이미 출장 완료 상태로, 예약 수정 및 취소가 불가능한 상태입니다.</c:if>
				<a class="highlight-button-dark2 btn btn-medium no-margin-right no-margin-bottom margin-two btn-round" href="/ash/getAshReservationAdminList">목록으로</a>
				</div>
            </div>
        </section>
    ...
    <div id="isUpdateReservation" style="background-color: rgba(0,0,0,0.4); width: 100%"  class="modal col-lg-3 col-md-4 col-sm-5 center-col text-center">
      ...
      <!-- 2. 예약수정 여부-->
	  <!-- 3. 출장확정 변경 여부-->
    <div id="isChangeOne" style="background-color: rgba(0,0,0,0.4); width: 100%"  class="modal col-lg-3 col-md-4 col-sm-5 center-col text-center">
      <div class="col-lg-3 col-md-6 col-sm-7 col-xs-11 center-col bg-white text-center modal-popup-main animated fadeIn"  style=" padding:35px; top: 30%">
                <p style="text-align: center;"><span style="font-size: 14pt;"><b><span style="font-size: 24pt;">알 림</span></b></span></p>
                <p class="borderline-gray"></p>
                <p style="text-align: center; line-height: 1.5;"><br />출장확정 하시겠습니까?</p>
                <p><br /></p>
            <div style="cursor:pointer; text-align: center;padding-bottom: 10px;padding-top: 10px;">
                <span id="changeOneOk" class="highlight-button-dark btn btn-medium no-margin pop_bt" style="font-size: 13pt;" >확인</span>
                <span id="changeOneNo" class="highlight-button-dark btn btn-medium no-margin pop_bt" style="font-size: 13pt;" >취소</span>
            </div>
      </div>
    </div>
      <!-- 3. 출장확정 변경 여부-->
	  <!-- 4. 출장완료 변경 여부-->
    <div id="isChangeTwo" style="background-color: rgba(0,0,0,0.4); width: 100%"  class="modal col-lg-3 col-md-4 col-sm-5 center-col text-center">
      <div class="col-lg-3 col-md-6 col-sm-7 col-xs-11 center-col bg-white text-center modal-popup-main animated fadeIn"  style=" padding:35px; top: 30%">
                <p style="text-align: center;"><span style="font-size: 14pt;"><b><span style="font-size: 24pt;">알 림</span></b></span></p>
                <p class="borderline-gray"></p>
                <p style="text-align: center; line-height: 1.5;"><br />출장확정 하시겠습니까?</p>
                <p><br /></p>
            <div style="cursor:pointer; text-align: center;padding-bottom: 10px;padding-top: 10px;">
                <span id="changeTwoOk" class="highlight-button-dark btn btn-medium no-margin pop_bt" style="font-size: 13pt;" >확인</span>
                <span id="changeTwoNo" class="highlight-button-dark btn btn-medium no-margin pop_bt" style="font-size: 13pt;" >취소</span>
            </div>
      </div>
    </div>
      <!-- 3. 출장완료 변경 여부-->
